...
def run(conf, logger, options):
    ...
    if hasattr(conf, "db_base"):
        try:
            osmosis_manager = modules.OsmOsisManager.OsmOsisManager(conf, conf.db_host, conf.db_user, conf.db_password, conf.db_base, conf.db_schema or conf.country, conf.db_persistant, logger)
        except:
            traceback.print_exc()
