...
class OsmOsisManager:
  ...
  def __init__(self, conf, db_host, db_user, db_password, db_base, db_schema, db_persistant, logger):
    self.conf = conf
    ...
    self.db_persistant = db_persistant
    self.logger = logger
  ...
  def clean_database(self, conf, no_clean):
    ...
    if no_clean and not self.db_persistant:
